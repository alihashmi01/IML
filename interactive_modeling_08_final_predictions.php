...
<div id="wrap">
 ...
 <?php 
	...
	?>
<form name="responseform" method="get" action="interactive_modeling_09_pedict_responses_submit.php" onsubmit="return responsevalidate()">
...
<?php			
	...
	{	
	   // look for the ones that we have not labelled  in predict column (these were not seen)
	   if ($data["articles"][$i]["predict"] =="notclassified" ){
		...
		echo "<u>keywords</u>:".substr($data["articles"][$i]["keywords"],0,400)."<br>";
		
		//BINARY CLASSIFICATION
		if ($data["articles"][$i]["predict_one"] ==$favtopic ){
			echo "<u>Category predicted</u>:<strong><FONT size=\"3\" COLOR=\"blue\"> ".$data["articles"][$i]["predict_one"]."</FONT></strong><br>";
		}
		else
		{
